...
def execute_command(ssh, command):
    ...
    time.sleep(2)  # Allow command execution time
...
def main():
    ...
    try:
        ...
        # # Step 2: Ensure we're in operational mode (interactive)
        # print("Entering Junos CLI mode...")
        ...
        # Step 3: Check if in operational mode
        output, error = execute_command(ssh, "show version")
        if "Command not found" in output:
            print("Not in operational mode. Switching to operational mode...")
            stdin, stdout, stderr = ssh.exec_command("cli")
            time.sleep(1)
            stdin.write('show version\n')
            stdin.flush()
